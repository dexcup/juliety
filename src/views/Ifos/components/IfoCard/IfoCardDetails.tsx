...
const IfoCardDetails: React.FC<IfoCardDetailsProps> = ({ ifo, publicIfoData }) => {
  ...
  console.log("raising and total is ", raisingAmount.toString(), totalAmount.toString())
      ...
      {isOpen && (
        <>
          ...
          <Box mb="24px">
            ...
            <Item>
              <Display>{TranslateString(586, 'FSTX to burn (USD)')}</Display>
              <Text>{cakeToBurn}</Text>
            </Item>
            <Item>
              <Display>{TranslateString(999, 'Total raised (% of target)')}</Display>
              <Text>{`${totalAmount.div(raisingAmount).times(100).toFixed(5)}%`}</Text>
            </Item>
          </Box>
